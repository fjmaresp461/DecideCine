...
class PreferencesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.setTitle(" 🤢<-           Elige tus gustos           ->🥰")
        setListener()
        if (userId != null) {
            loadUserPreferencesFromFirebase(userId)
        }
    }
    ...
    private fun savePreferencesToFirebase(userId: String) {
    ...
    private fun checkValues() {
        ...
        if (userId != null) {
            savePreferencesToFirebase(userId)
        }
    }
}
